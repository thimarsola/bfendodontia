...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){const host=$(location).attr("host");if(host=="localhost"){var path=$(location).attr("origin").concat('/').concat($(location).attr("pathname").split("/",2).reverse().join(''))}else{var path=$(location).attr("origin")}
const file='wp-content/themes/bfendodontia/source/Support/Sender.php';$('#form').submit(function(){$(".contact__row__form__status").removeClass("d-none");$.ajax({url:path.concat('/').concat(file),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.contact__row__form__status').append(data);setTimeout(function(){$(".contact__row__form__status").addClass("d-none")},3000)},error:function(){$('.contact__row__form__status').append('Erro')}});return!1})})